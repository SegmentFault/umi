    ...
    Program: {
      enter(path) {
        ...
        if (_utils().t.isIdentifier(defaultExport)) {
          ...
          const resolver = _propertyResolver.NODE_RESOLVERS.find((resolver) =>
...
function findAssignmentExpressionProps(opts) {
  ...
  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done; ) {
      ...
      if (
        _utils().t.isAssignmentExpression(node) &&
        _utils().t.isMemberExpression(node.left) &&
        _utils().t.isIdentifier(node.left.object) &&
        node.left.object.name === opts.name
      ) {
        const resolver = _propertyResolver.NODE_RESOLVERS.find((resolver) =>
          resolver.is(_utils().t.isAssignmentExpression(node) && node.right),
        );
