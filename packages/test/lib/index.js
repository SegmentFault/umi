...
var _createDefaultConfig = _interopRequireDefault(
  require('./createDefaultConfig/createDefaultConfig'),
);
...
function _ref() {
  _ref = _asyncToGenerator(function* (args) {
    ...
    const userJestConfig =
      (0, _fs().existsSync)(userJestConfigFile) && require(userJestConfigFile);
    ...
    const packageJestConfig =
      (0, _fs().existsSync)(packageJSONPath) && require(packageJSONPath).jest;

    debug(
      `jest config from package.json: ${JSON.stringify(packageJestConfig)}`,
    ); // Merge configs
    // user config and args config could have value function for modification

    const config = (0, _utils().mergeConfig)(
      (0, _createDefaultConfig.default)(cwd, args),
      packageJestConfig,
      userJestConfig,
    );
    ...
    const result = yield (0, _jest().runCLI)(
      _objectSpread(
        {
          // @ts-ignore
          _: args._ || [],
