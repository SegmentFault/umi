...
function _getConfig() {
  _getConfig = _asyncToGenerator(function* (opts) {
    const cwd = opts.cwd,
      ...
      bundleImplementor =
        _opts$bundleImplement === void 0
          ? _webpack().default
          : _opts$bundleImplement,
    ...
    webpackConfig.devtool(
      isDev // devtool 设为 false 时不 fallback 到 cheap-module-source-map
        ? devtool === false
          ? false
    ...
    webpackConfig.output
      ...
      .publicPath(config.publicPath) // remove this after webpack@5
      // free memory of assets after emitting
      .futureEmitAssets(true)
      .pathinfo(isDev || disableCompress); // resolve
    // prettier-ignore
    ...
    const _getTargetsAndBrowser = (0,
      _bundlerUtils().getTargetsAndBrowsersList)({
        config,
        type,
      }),
    ...
    webpackConfig.module
      ...
      .include.add([
        cwd, // import module out of cwd using APP_ROOT
    ...
    if (process.env.UMI_DIR) {
      // prettier-ignore
      webpackConfig.module.rule('js-for-umi-dist').test(/\.(js|mjs|jsx)$/).include.add((0, _path().join)(process.env.UMI_DIR, 'dist', 'index.esm.js')).end().use('babel-loader').loader(require.resolve('babel-loader')).options(babelOpts);
    } // prettier-ignore
    ...
    if (nodeModulesTransform.type === 'all') {
      const exclude = _utils().lodash.uniq([
        ..._nodeModulesTransform.TYPE_ALL_EXCLUDE,
      ...
      const pkgs = _objectSpread(
        _objectSpread(
          {},
          (0, _nodeModulesTransform.es5ImcompatibleVersionsToPkg)(),
        ),
        (0, _nodeModulesTransform.excludeToPkgs)({
    ...
    webpackConfig.module
      ...
      .test(/\.(txt|text|md)$/)
      .use('raw-loader')
      .loader(require.resolve('raw-loader')); // css
    ...
    if (process.env.PROGRESS !== 'none') {
      webpackConfig.plugin('progress').use(require.resolve('webpackbar'), [
        config.ssr
          ? {
              name:
                type === _types().BundlerConfigType.ssr ? 'Server' : 'Client',
            }
          : {},
      ]);
    } // copy
    ...
    const copyPatterns = [
      (0, _fs().existsSync)((0, _path().join)(cwd, 'public')) && {
        ...
        if (disableCompress) {
          ...
          webpackConfig.optimization
            ...
            .use(require.resolve('terser-webpack-plugin'), [
              {
                terserOptions: (0, _utils().deepmerge)(
                  _terserOptions.default,
