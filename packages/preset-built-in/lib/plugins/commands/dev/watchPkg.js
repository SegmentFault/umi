...
function getUmiPlugins(opts) {
  return Object.keys(
    _objectSpread(
      _objectSpread({}, opts.pkg.dependencies),
      opts.pkg.devDependencies,
    ),
  ).filter((name) => {
    return (
      (0, _core().isPluginOrPreset)(_core().PluginType.plugin, name) ||
      (0, _core().isPluginOrPreset)(_core().PluginType.preset, name)
    );
  });
}
...
function watchPkgs(opts) {
  ...
  if (
    (0, _utils().winPath)(opts.cwd) !== (0, _utils().winPath)(process.cwd())
  ) {
    unwatchs.push(
      watchPkg({
        cwd: process.cwd(),
