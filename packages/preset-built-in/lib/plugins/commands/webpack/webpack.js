    ...
    fn() {
      return _asyncToGenerator(function* () {
        ...
        let config = bundleConfigs.filter((bundleConfig) => {
          ...
          return (_bundleConfig$entry = bundleConfig.entry) === null ||
            _bundleConfig$entry === void 0
            ? void 0
            : _bundleConfig$entry.umi;
        })[0];
        (0, _assert().default)(config, `No valid config found with umi entry.`);

        if (api.args.rule) {
          config = config.module.rules.find(
            (r) => r.__ruleNames[0] === api.args.rule,
          );
        } else if (api.args.plugin) {
          config = config.plugins.find(
            (p) => p.__pluginName === api.args.plugin,
          );
