...
function _getBundleAndConfigs() {
  _getBundleAndConfigs = _asyncToGenerator(function* ({ api, port }) {
    // bundler
    ...
    function _getConfig() {
      _getConfig = _asyncToGenerator(function* ({ type }) {
...
function cleanTmpPathExceptCache({ absTmpPath }) {
...
function printFileSizes(stats, dir) {
  ...
  const assets = json.assets
    ? json.assets // @ts-ignore
    : json === null || json === void 0
    ? void 0
    : (_json$children = json.children) === null || _json$children === void 0
    ? void 0
    : _json$children.reduce(
        (acc, child) =>
          acc.concat(
            child === null || child === void 0 ? void 0 : child.assets,
          ),
        [],
      );
  ...
  ui.div(
    makeRow(
      _utils().chalk.cyan.bold(`File`),
      _utils().chalk.cyan.bold(`Size`),
      _utils().chalk.cyan.bold(`Gzipped`),
    ) +
      `\n\n` +
      (orderedAssets === null || orderedAssets === void 0
        ? void 0
        : orderedAssets
            .map((asset) =>
              makeRow(
                /js$/.test(asset.name)
                  ? asset.suggested // warning for large bundle
                    ? _utils().chalk.yellow((0, _path().join)(dir, asset.name))
                    : _utils().chalk.green((0, _path().join)(dir, asset.name))
                  : _utils().chalk.blue((0, _path().join)(dir, asset.name)),
                filesize(asset.size),
  ...
  if (
    orderedAssets === null || orderedAssets === void 0
      ? void 0
      : orderedAssets.some((asset) => asset.suggested)
  ) {
