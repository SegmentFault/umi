 ...
 * @param api
 */
const onBuildComplete = (api, _isTest = false) =>
  /*#__PURE__*/ (function () {
    var _ref = _asyncToGenerator(function* ({ err, stats }) {
...
var _default = (api) => {
  ...
  api.onStart(() => {
    ...
    (0, _assert().default)(
      ((_api$config$history = api.config.history) === null ||
      _api$config$history === void 0
        ? void 0
        : _api$config$history.type) !== 'hash',
    ...
    if (!process.env.WATCH_IGNORED) {
      ...
      const absOutputPath = (0, _utils().winPath)(
        path().join(api.cwd, outputPath, '/'),
      );
      process.env.WATCH_IGNORED = `(node_modules|${absOutputPath}(?!${_constants.OUTPUT_SERVER_FILENAME}))`;
    }
  }); // 再加一个 webpack instance

  api.modifyBundleConfigs(
    /*#__PURE__*/ (function () {
      var _ref4 = _asyncToGenerator(function* (memo, { getConfig }) {
        return [
          ...memo,
          yield getConfig({
  ...
  api.onGenerateFiles(
    /*#__PURE__*/ _asyncToGenerator(function* () {
      var _api$config$dynamicIm,
        _api$config$dynamicIm2,
        _api$config$ssr$mode,
        _api$config$ssr,
        _api$config$ssr2,
        _api$config$ssr3,
        _api$config$ssr4;

      const serverTpl = path().join(
        (0, _utils().winPath)(__dirname),
      ...
      api.writeTmpFile({
        ...
        content: _utils().Mustache.render(serverContent, {
          ...
          RuntimePath: (0, _utils().winPath)(
            path().dirname(require.resolve('@umijs/runtime/package.json')),
          ),
          Renderer: (0, _utils().winPath)(
            require.resolve('./templates/renderServer/renderServer'),
          ),
          RuntimePolyfill: (0, _utils().winPath)(
            require.resolve('regenerator-runtime/runtime'),
          ),
          loadingComponent:
            ((_api$config$dynamicIm = api.config.dynamicImport) === null ||
            _api$config$dynamicIm === void 0
              ? void 0
              : _api$config$dynamicIm.loading) &&
            (0, _utils().winPath)(
              (_api$config$dynamicIm2 = api.config.dynamicImport) === null ||
                _api$config$dynamicIm2 === void 0
                ? void 0
                : _api$config$dynamicIm2.loading,
            ),
          DynamicImport: !!api.config.dynamicImport,
          Utils: (0, _utils().winPath)(require.resolve('./templates/utils')),
          Mode:
            (_api$config$ssr$mode =
              (_api$config$ssr = api.config.ssr) === null ||
              _api$config$ssr === void 0
                ? void 0
                : _api$config$ssr.mode) !== null &&
            _api$config$ssr$mode !== void 0
              ? _api$config$ssr$mode
              : 'string',
          MountElementId: api.config.mountElementId,
          StaticMarkup: !!((_api$config$ssr2 = api.config.ssr) === null ||
          _api$config$ssr2 === void 0
            ? void 0
            : _api$config$ssr2.staticMarkup),
          // @ts-ignore
          ForceInitial: !!((_api$config$ssr3 = api.config.ssr) === null ||
          _api$config$ssr3 === void 0
            ? void 0
            : _api$config$ssr3.forceInitial),
          RemoveWindowInitialProps: !!((_api$config$ssr4 = api.config.ssr) ===
            null || _api$config$ssr4 === void 0
            ? void 0
            : _api$config$ssr4.removeWindowInitialProps),
          Basename: api.config.base,
          PublicPath: api.config.publicPath,
          ManifestFileName: api.config.manifest
            ? api.config.manifest.fileName || 'asset-manifest.json'
            : '',
          DEFAULT_HTML_PLACEHOLDER: (0, _serializeJavascript().default)(
            defaultHTML,
          ),
        }),
      });
      const clientExportsContent = fs().readFileSync(
        path().join(
          (0, _utils().winPath)(__dirname),
          `templates/${_constants.CLIENT_EXPORTS}.tpl`,
        ),
        'utf-8',
      );
      api.writeTmpFile({
        ...
        content: _utils().Mustache.render(clientExportsContent, {
          SSRUtils: (0, _utils().winPath)(
            require.resolve('@umijs/utils/lib/ssr'),
          ),
        }),
      });
    }),
    ...
    /*#__PURE__*/ (function () {
      var _ref6 = _asyncToGenerator(function* (memo, opts) {
        ...
        if (
          ...
          const chunkArr = [];
          const routes = yield api.getRoutes();
          const matchedRoutes = (0, _reactRouterConfig().matchRoutes)(
            routes,
            route.path,
          );

          const chunks = _utils().lodash.uniq(
            matchedRoutes.map((matchedRoute) =>
              matchedRoute.route.component
                ? (0, _utils().routeToChunkName)({
    ...
    config.devServer.writeToDisk = (filePath) => {
      ...
      const manifestFile =
        ((_api$config2 = api.config) === null || _api$config2 === void 0
          ? void 0
          : (_api$config2$manifest = _api$config2.manifest) === null ||
            _api$config2$manifest === void 0
          ? void 0
          : _api$config2$manifest.fileName) || 'asset-manifest.json';
      const regexp = new RegExp(
        `(${_constants.OUTPUT_SERVER_FILENAME}|${_constants.OUTPUT_SERVER_TYPE_FILENAME}|${manifestFile})$`,
      );
  ...
  api.onDevCompileDone(() => {
    const serverExp = new RegExp(
      _utils().lodash.escapeRegExp(_constants.OUTPUT_SERVER_FILENAME),
    ); // clear require cache

    for (
      var _i2 = 0, _Object$keys = Object.keys(require.cache);
  ...
  api.modifyDevHTMLContent(
    /*#__PURE__*/ (function () {
      var _ref7 = _asyncToGenerator(function* (defaultHtml, { req }) {
        var _api$config3;

        // umi dev to enable server side render by default
        const _ref8 =
            ((_api$config3 = api.config) === null || _api$config3 === void 0
              ? void 0
              : _api$config3.ssr) || {},
          mode = _ref8.mode,
          _ref8$devServerRender = _ref8.devServerRender,
          devServerRender =
            _ref8$devServerRender === void 0 ? true : _ref8$devServerRender;
        ...
        try {
          ...
          const _yield$render = yield render({
              ...
              mountElementId:
                (_api$config4 = api.config) === null || _api$config4 === void 0
                  ? void 0
                  : _api$config4.mountElementId,
            }),
          ...
          console.log(
            `[SSR] ${mode === 'stream' ? 'stream' : ''} render ${
              req.url
            } start: ${(endTime - startTime).toFixed(2)}ms`,
          );
    ...
    /*#__PURE__*/ (function () {
      var _ref9 = _asyncToGenerator(function* (config, opts) {
        ...
        if (type === _types().BundlerConfigType.ssr) {
          ...
          config.output
            .filename(_constants.OUTPUT_SERVER_FILENAME) // avoid using `require().default`, just using `require()`
            .libraryExport('default')
            .chunkFilename('[name].server.js')
            .libraryTarget('commonjs2'); // disable *.server.chunk.js, routes avoid dynamic and require in different mode.

          config
            .plugin('limit-chunk')
            .use(_bundlerWebpack().webpack.optimize.LimitChunkCountPlugin, [
              {
                maxChunks: 1,
              },
            ]);
            ...
            [
              {
                ...
                content: `import { IServerRender } from 'umi';${
                  _os().EOL
                }export = render;${_os().EOL}export as namespace render;${
                  _os().EOL
