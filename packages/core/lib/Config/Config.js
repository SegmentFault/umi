...
class Config {
  constructor(opts) {
    ...
    if (Array.isArray(opts.configFiles)) {
      // 配置的优先读取
      this.configFiles = _utils().lodash.uniq(
        opts.configFiles.concat(this.configFiles),
      );
    }
  }
    ...
    return _asyncToGenerator(function* () {
      ...
      let defaultConfig = pluginIds.reduce((memo, pluginId) => {
        const _this$service$plugins = _this.service.plugins[pluginId],
          ...
          config =
            _this$service$plugins2 === void 0 ? {} : _this$service$plugins2;
  ...
  getConfig({ defaultConfig }) {
    (0, _assert().default)(
      this.service.stage >= _enums.ServiceStage.pluginReady,
    ...
    pluginIds.forEach((pluginId) => {
      const _this$service$plugins3 = this.service.plugins[pluginId],
        ...
        config =
          _this$service$plugins4 === void 0 ? {} : _this$service$plugins4; // recognize as key if have schema config
    ...
    if (configFile) {
      ...
      if (process.env.UMI_ENV) {
        ...
        const fileNameWithoutExt = envConfigFileName.replace(
          (0, _path().extname)(envConfigFileName),
          '',
        );
        envConfigFile =
          (_getFile = (0, _utils().getFile)({
            base: this.cwd,
            fileNameWithoutExt,
            type: 'javascript',
          })) === null || _getFile === void 0
            ? void 0
            : _getFile.filename;
      ...
      const files = [
        ...
        .filter((f) => !!f)
        .map((f) => (0, _path().join)(this.cwd, f))
        .filter((f) => (0, _fs().existsSync)(f)); // clear require cache and set babel register
  ...
  requireConfigs(configFiles) {
    return configFiles.map((f) =>
      (0, _utils().compatESModuleRequire)(require(f)),
    );
  }
  ...
  getConfigFile() {
    ...
    const configFile = this.configFiles.find((f) =>
      (0, _fs().existsSync)((0, _path().join)(this.cwd, f)),
    );
  ...
  getWatchFilesAndDirectories() {
    ...
    const configDir = (0, _utils().winPath)(
      (0, _path().join)(this.cwd, 'config'),
    );
    ...
    watcher.on('all', (event, path) => {
      ...
      Object.keys(this.service.plugins).forEach((pluginId) => {
        const _this$service$plugins5 = this.service.plugins[pluginId],
          ...
          config =
            _this$service$plugins6 === void 0 ? {} : _this$service$plugins6; // recognize as key if have schema config
