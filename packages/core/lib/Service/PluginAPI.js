...
class PluginAPI {
  ...
  registerPresets(presets) {
    (0, _assert().default)(
      this.service.stage === _enums.ServiceStage.initPresets,
    ...
    const extraPresets = presets.map((preset) => {
      return (0, _pluginUtils.isValidPlugin)(preset)
        ? preset
        : (0, _pluginUtils.pathToObj)({
  ...
  registerPlugins(plugins) {
    (0, _assert().default)(
      this.service.stage === _enums.ServiceStage.initPresets ||
        this.service.stage === _enums.ServiceStage.initPlugins,
    ...
    const extraPlugins = plugins.map((plugin) => {
      return (0, _pluginUtils.isValidPlugin)(plugin)
        ? plugin
        : (0, _pluginUtils.pathToObj)({
  ...
  registerMethod({ name, fn, exitsError = true }) {
    ...
    this.service.pluginMethods[name] =
      fn || // 这里不能用 arrow function，this 需指向执行此方法的 PluginAPI
