...
function getPluginsOrPresets(type, opts) {
  ...
  return [
    ...
    ...Object.keys(opts.pkg.devDependencies || {})
      ...
      .filter(isPluginOrPreset.bind(null, type)), // user config
    ...(opts[
...
function nameToKey(name) {
  return name
    .split('.')
    .map((part) => _utils().lodash.camelCase(part))
    .join('.');
}
...
function pathToObj({ type, path, cwd }) {
  ...
  (0, _assert().default)(
    (0, _fs().existsSync)(path),
    `${type} ${path} not exists, pathToObj failed`,
  );
  ...
  if (pkgJSONPath) {
    ...
    isPkgPlugin =
      (0, _utils().winPath)(
        (0, _path().join)(
          (0, _path().dirname)(pkgJSONPath),
          pkg.main || 'index.js',
        ),
      ) === (0, _utils().winPath)(path);
  }
  ...
  if (isPkgPlugin) {
    ...
    (0, _utils().winPath)(path).startsWith((0, _utils().winPath)(cwd))
  ) {
    ...
    id = (0, _utils().winPath)(
      (0, _path().join)(
        pkg.name,
        (0, _path().relative)((0, _path().dirname)(pkgJSONPath), path),
      ),
    );
  } else {
  ...
  const key = isPkgPlugin
    ...
    : nameToKey((0, _path().basename)(path, (0, _path().extname)(path)));
  return {
    id,
    key,
