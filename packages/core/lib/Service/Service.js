...
class Service extends _events().EventEmitter {
  constructor(opts) {
    ...
    (0, _assert().default)(
      (0, _fs().existsSync)(this.cwd),
      `cwd ${this.cwd} does not exist.`,
  ...
  init() {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (_this.config.outputPath) {
        _this.paths.absOutputPath = (0, _path().join)(
          _this.cwd,
          _this.config.outputPath,
        );
      }
  ...
  initPreset(preset) {
    ...
    return _asyncToGenerator(function* () {
      ...
      const _yield$_this3$applyAP = yield _this3.applyAPI({
        ...
        defaultConfigs = _objectWithoutProperties(_yield$_this3$applyAP, [
          'presets',
          'plugins',
...
${name} from ${plugin.path} register failed.`);
  ...
  runCommand({ name, args = {} }) {
    ...
    return _asyncToGenerator(function* () {
      (0,
      _assert()
        .default)(_this7.stage >= _enums.ServiceStage.init, `service is not initialized.`);
      args._ = args._ || []; // shift the command itself

      if (args._[0] === name) args._.shift();
      const command =
        typeof _this7.commands[name] === 'string'
          ? _this7.commands[_this7.commands[name]]
          : _this7.commands[name];
      (0,
      _assert()
        .default)(command, `run command failed, command ${name} does not exists.`);
      const fn = command.fn;
      return fn({
        args,
      });
