...
class Html {
  ...
  getHtmlPath(path) {
    ...
    if (
      ((_this$config$exportSt = this.config.exportStatic) === null ||
      _this$config$exportSt === void 0
        ? void 0
        : _this$config$exportSt.htmlSuffix) ||
      path === 'index.html'
    ) {
      return `${path}`;
    } else {
  ...
  getRelPathToPublicPath(path) {
    ...
    return (
      Array(
        (
          (_this$config$exportSt2 = this.config.exportStatic) === null ||
          _this$config$exportSt2 === void 0
            ? void 0
            : _this$config$exportSt2.htmlSuffix
        )
          ? len
          : len - 1,
      ).join('../') || './'
  ...
  getAsset(opts) {
    ...
    if (
      (_this$config$exportSt3 = this.config.exportStatic) === null ||
      _this$config$exportSt3 === void 0
        ? void 0
        : _this$config$exportSt3.dynamicRoot
    ) {
  ...
  getContent(args) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (tplPath) {
        (0, _assert().default)(
          (0, _fs().existsSync)(tplPath),
          `getContent() failed, tplPath of ${tplPath} not exists.`,
        );
      }

      const tpl = (0, _fs().readFileSync)(
        tplPath || (0, _path().join)(__dirname, 'document.ejs'),
        'utf-8',
      );
      ...
      cssFiles.forEach((file) => {
        $('head').append(
          `<link rel="stylesheet" href="${_this.getAsset({
            file,
            path: route.path,
          })}" />`,
        );
      ...
      if (!$(`#${mountElementId}`).length) {
        ...
        (0, _assert().default)(
          bodyEl.length,
          `<body> not found in html template.`,
        );
