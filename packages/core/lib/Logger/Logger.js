...
class Logger extends _Common.default {
  ...
  printUmiError(e, opts = {}) {
    ...
    console.error(
      `\n${_utils().chalk.bgRed.black('ERROR CODE')} ${_utils().chalk.red(
        code,
      )}`,
    );
    ...
    if (!detailsOnly) {
      console.error(
        `\n${_utils().chalk.bgRed.black('ERROR')} ${_utils().chalk.red(
          e.message || message,
        )}`,
      );
    }
    ...
    if (lang === 'zh-CN') {
      console.error(
        ...
        _marked().default)(details['zh-CN'])}`,
      );
    } else {
      console.error(
        `\n${_utils().chalk.bgMagenta.black(' DETAILS ')}\n\n${(0,
        _marked().default)(details.en)}`,
      );
    }

    if (!detailsOnly && e.stack) {
      console.error(
        `${_utils().chalk.bgRed.black(' STACK ')}\n\n${e.stack
          .split('\n')
          .slice(1)
          .join('\n')}`,
      );
  ...
  profile(id, message) {
    ...
    if (this.profilers[id]) {
      ...
      msg = `${this.PROFILE} ${_utils().chalk.cyan(
        `└ ${namespace}`,
      )} Completed in ${this.formatTiming(time - timeEnd)}`;
      console.log(msg);
    } else {
      msg = `${this.PROFILE} ${_utils().chalk.cyan(`┌ ${namespace}`)} ${
        message || ''
      }`;
      console.log(msg);
    }
