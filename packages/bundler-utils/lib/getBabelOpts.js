...
function getBasicBabelLoaderOpts({ cwd }) {
  const prefix = (0, _fs().existsSync)((0, _path().join)(cwd, 'src'))
    ? (0, _path().join)(cwd, 'src')
    : cwd;
  return {
    ...
    cacheDirectory:
      process.env.BABEL_CACHE !== 'none'
        ? (0, _utils().winPath)(`${prefix}/.umi/.cache/babel-loader`)
        : false,
  };
}
...
function getBabelOpts({ cwd, config, presetOpts }) {
...
function getBabelDepsOpts({ env, cwd, config }) {
  return _objectSpread(
    _objectSpread(
      {},
      getBasicBabelLoaderOpts({
